...
bp <- read.csv('samples_basepair.txt', header = FALSE, sep = '\t')  ## virus identified from contigs > 5000 bp. Therefore, the viral richness data may be covariated by the number of contig > 5000
...
metadata_animal_performance <- read_csv("metadata_animal_performance.csv")
metadata_ecology <- read_csv("metadata_ecology.csv")  


table_beta_performance <- fread("for_beta_diversity_performance.csv")
...
```{r}
richness_box <- richness_ready_performance %>% ggplot() + 
  aes(x=treatment, y= richness_normalized) + geom_boxplot(aes(color = animal)) +
  facet_wrap(vars(trait, liter), scales = "free", labeller = function (labels) {   # reduce space between the facet legend 
               labels <- lapply(labels, as.character)           
               list(do.call(paste, c(labels, list(sep = "\n"))))
             }) + 
  labs(x = "", y = "Richness per billion bp") +  labs(color = "Species") +
  geom_jitter(size = 1, width = 0.2, height = 0.2, aes(color = animal)) + 
...
virome_richness <- richness_box +
   geom_curve(data=tibble(liter="Stewart et al., 2018", trait = "Concentrate level"), aes(x=1, xend=2, y=175, yend=175), curvature = -0.1, color = "#FF0000") +
  geom_text(data=tibble(liter="Stewart et al., 2018", trait = "Concentrate level"), aes(x=1.5, y=190, label = "**" ), color = "#FF0000", size=5) + 
  geom_curve(data=tibble(liter="Lu et al., 2019", trait = "Dietary composition"), aes(x=1, xend=3, y=125, yend=125), curvature = -0.1, color = "#FF0000") +
  geom_text(data=tibble(liter="Lu et al., 2019", trait = "Dietary composition"), aes(x=2, y=135, label = "***" ), color = "#FF0000", size=5) + 
  geom_curve(data=tibble(liter="Lu et al., 2019", trait = "Dietary composition"), aes(x=2, xend=3, y=105, yend=105), curvature = -0.1, color = "#FF0000") +
  geom_text(data=tibble(liter="Lu et al., 2019", trait = "Dietary composition"), aes(x=2.5, y=110, label = "*" ), color = "#FF0000", size=5) + 
  geom_curve(data=tibble(liter="Shi et al., 2014", trait = "Methane emission"), aes(x=1, xend=2, y=102, yend=102), curvature = -0.1, color = "#FF0000") +
  geom_text(data=tibble(liter="Shi et al., 2014", trait = "Methane emission"), aes(x=1.5, y=107, label = "***" ), color = "#FF0000", size=5) +
  geom_curve(data=tibble(liter="Li et al., 2020", trait = "Dietary composition"), aes(x=1, xend=2, y=135, yend=135), curvature = -0.1, color = "#FF0000") +
  geom_text(data=tibble(liter="Li et al., 2020", trait = "Dietary composition"), aes(x=1.5, y=145, label = "***" ), color = "#FF0000", size=5) +
  geom_curve(data=tibble(liter="Li et al., 2020", trait = "Dietary composition"), aes(x=3, xend=4, y=45, yend=45), curvature = -0.1, color = "#FF0000") +
  geom_text(data=tibble(liter="Li et al., 2020", trait = "Dietary composition"), aes(x=3.5, y=55, label = "***" ), color = "#FF0000", size=5) +
  geom_curve(data=tibble(liter="Anderson et al., 2021", trait = "Average daily gain"), aes(x=1, xend=2, y=145, yend=145), curvature = -0.1, color = "#FF0000") +
  geom_text(data=tibble(liter="Anderson et al., 2021", trait = "Average daily gain"), aes(x=1.5, y=155, label = "*" ), color = "#FF0000", size=5)  
            
virome_richness
ggsave("virome_richness.svg", width = 9, height = 6)
```
...
```{r}
function_pcoa <- function(literature){ PCOA_df %>% filter(liter == literature) %>% ggplot(aes(x=Axis.1, y=Axis.2, color=treatment)) + geom_point() + facet_wrap(vars(trait, liter), labeller = function (labels) {   # reduce space between the facet legend 
  ...
  theme(strip.text.x = element_text(size = 8)) + # Change font size of facet strip
   theme_minimal() +
    theme(legend.position = "bottom") +
...
pcoa_plots[["Shi et al., 2014"]] <- pcoa_plots[["Shi et al., 2014"]] + annotate("text", -Inf, -Inf, hjust = 0, vjust = 0, label = "Permanova: ***", size = 3)  # hjust and vjust are defined between 0 and 1, min to max
pcoa_plots[["Li et al., 2019"]] <- pcoa_plots[["Li et al., 2019"]] + annotate("text", -Inf, -Inf, hjust = 0, vjust = 0, label = "Permanova: *", size = 3)
pcoa_plots[["Li et al., 2020"]] <- pcoa_plots[["Li et al., 2020"]] + annotate("text", -Inf, -Inf, hjust = 0, vjust = 0, label = "Permanova: ***", size = 3) 
pcoa_plots[["Stewart et al., 2018"]] <- pcoa_plots[["Stewart et al., 2018"]] + annotate("text",  -Inf, -Inf, hjust = 0, vjust = 0, label = "Permanova: ***", size = 3) 
pcoa_plots[["Xue et al., 2020"]] <- pcoa_plots[["Xue et al., 2020"]] + annotate("text",  -Inf, -Inf, hjust = 0, vjust = 0, label = "Permanova: *", size = 3) 
pcoa_plots[["Lu et al., 2019"]] <- pcoa_plots[["Lu et al., 2019"]] + annotate("text",  -Inf, -Inf, hjust = 0, vjust = 0, label = "Permanova: ***", size = 3) 
...
pcoa_final
ggsave("virome_pcoa.svg", width = 9, height = 6)
```
...
#plot_legend_li2020 <- PCOA_df %>% filter(liter == "Li et al., 2020") %>% ggplot(aes(x=Axis.1, y=Axis.2, color=treatment)) + geom_point() + labs(color = "Li et al., 2020") + theme(legend.title = element_text(size = 9)) 

#plot_legend_Stergiadis <- PCOA_df %>% filter(liter == "Stergiadis et al., 2021") %>% ggplot(aes(x=Axis.1, y=Axis.2, color=treatment)) + geom_point() + labs(color = "Stergiadis et al., 2021") + theme(legend.title = element_text(size = 9)) 

#plot_legend_lu <- PCOA_df %>% filter(liter == "Lu et al., 2019") %>% ggplot(aes(x=Axis.1, y=Axis.2, color=treatment)) + geom_point() + labs(color = "Lu et al., 2019") + theme(legend.title = element_text(size = 9)) 

#plot_legend_highorlow <- PCOA_df %>% filter(liter == "Shi et al., 2014") %>% ggplot(aes(x=Axis.1, y=Axis.2, color=treatment)) + geom_point() + labs(color = "Others") + theme(legend.title = element_text(size = 9))

#legend_li2020 <- g_legend(plot_legend_li2020) 

#legend_Stergiadis <- g_legend(plot_legend_Stergiadis)

#legend_lu <- g_legend(plot_legend_lu)

#legend_highorlow <- g_legend(plot_legend_highorlow) 

#legend_ready <- plot_grid(legend_li2020, legend_Stergiadis, legend_lu, legend_highorlow, ncol = 1, align = "v")

#grid.arrange(pcoa_final, legend_ready , ncol = 2, widths = c(4/5,1/5))
