...
micro_box_diet <- micro_ready_performance %>% filter(study %in% diet_study) %>% ggplot() + 
  ...
  labs(x = "", y = expression("Microdiversity ("*pi*")")) +  labs(color = "Species") +
  geom_jitter(size = 1, width = 0.2,  aes(color = animal)) +
  theme_minimal()+
...
micro_box_diet_anno <- micro_box_diet +
  ...
  geom_text(data=tibble(trait = "Concentrate level"), aes(x=1.5, y=0.0135, label = "***" ), size=4) + 
  geom_curve(data=tibble(trait = "Dietary composition (goat)"), aes(x=1, xend=3, y=0.0063, yend=0.0063), curvature = -0.1) +
  geom_text(data=tibble(trait = "Dietary composition (goat)"), aes(x=2, y=0.0065, label = "***" ), size=4) + 
  geom_curve(data=tibble(trait = "Dietary composition (goat)"), aes(x=2, xend=3, y=0.005, yend=0.005), curvature = -0.1) +
  geom_text(data=tibble(trait = "Dietary composition (goat)"), aes(x=2.5, y=0.00515, label = "**" ), size=4) +
   geom_curve(data=tibble(trait = "Dietary composition (goat)"), aes(x=1, xend=2, y=0.006, yend=0.006), curvature = -0.1) +
  geom_text(data=tibble(trait = "Dietary composition (goat)"), aes(x=1.5, y=0.00615, label = "***" ), size=4) +
  geom_curve(data=tibble(trait = "Dietary composition (cattle)"), aes(x=3, xend=4, y=0.0075, yend=0.0075), curvature = -0.1) +
  geom_text(data=tibble(trait = "Dietary composition (cattle)"), aes(x=3.5, y=0.0078, label = "*" ), size=4) 

svg("/fs/ess/PAS0439/MING/virome_ecology/results/inStrain_RVD/virome_micro_diet.svg", width = 8, height = 2.2)
micro_box_diet_anno 
dev.off()
...
micro_box_phenotype_anno <- micro_box_phenotype +
  ...
  geom_text(data=tibble(trait = "Methane emission"), aes(x=1.5, y=0.00835, label = "*" ), size=4) +
  geom_curve(data=tibble(trait = "Milk protein yield"), aes(x=1, xend=2, y=0.0215, yend=0.0215), curvature = -0.1) +
  geom_text(data=tibble(trait = "Milk protein yield"), aes(x=1.5, y=0.0225, label = "**" ), size=4)    

svg("/fs/ess/PAS0439/MING/virome_ecology/results/inStrain_RVD/virome_micro_pheno.svg", width = 8, height = 3)
...
ggsave("/fs/ess/PAS0439/MING/virome_ecology/results/inStrain_RVD/virome_micro_legend.svg", width = 8, height = 1)
```
```